...
estimatedTime = Number((Parts.findOne().quantity / Parts.findOne().cavitation) *"23");
 ...
 displayHours = Number(moment(Parts.find().fetch().pop().timestamp.toString()).format("H"));
 ...
 displayHours = displayHours + Number(moment(Parts.find().fetch().pop().timestamp.toString()).format("m"))
//This code will add the estimated time to complete to the minutes
 displayHours = displayHours + estimatedTimeMinutes //This adds minutes from estimated time
  ...
  past= Number(moment(Parts.find().fetch().pop().timestamp.toString()).format("H"));
    past = past*60;
  past = past + Number(moment(Parts.find().fetch().pop().timestamp.toString()).format("m"))
    numerator=now-past;
